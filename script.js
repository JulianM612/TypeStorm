...
const API_NINJAS_KEY = 'YOUR_API_KEY'; // <<< REPLACE WITH YOUR ACTUAL API KEY
...
async function loadPassageFromAPI() {
    ...
    try {
        ...
        } else { throw new Error("API response not in expected format or empty quote content."); }
    } catch (error) {
...
// NEW: Function to update the current word highlighting
function updateCurrentWordHighlight() {
    if (!passageCharsSpans || passageCharsSpans.length === 0) return;
    passageCharsSpans.forEach(span => span.classList.remove('current-word'));
    if (currentCharIndex >= currentPassageText.length || currentCharIndex < 0) return;

    let wordStartIndex = currentCharIndex;
    while (wordStartIndex > 0 && currentPassageText[wordStartIndex - 1] !== ' ') {
        wordStartIndex--;
    }
    let wordEndIndex = currentCharIndex;
    // Ensure wordEndIndex does not go out of bounds for currentPassageText
    while (wordEndIndex < currentPassageText.length -1  && currentPassageText[wordEndIndex + 1] !== ' ') {
        wordEndIndex++;
    }
    for (let i = wordStartIndex; i <= wordEndIndex; i++) {
        if (passageCharsSpans[i]) {
            passageCharsSpans[i].classList.add('current-word');
        }
    }
}
...
async function resetGame() {
    ...
    if (passageCharsSpans.length > 0) { passageCharsSpans[0].classList.add('current'); }
    updateCurrentWordHighlight(); // Initial word highlight
...
function processTypedCharacter(typedChar, isBackspace = false) {
    if (isBackspace) {
        ...
        if (typedChar === expectedChar) {
            ...
            if (!passageContainer.classList.contains('shake-error')) {
                ...
                setTimeout(() => { passageContainer.classList.remove('shake-error'); }, 300);
            }
        }
        currentCharIndex++;
        if (currentCharIndex < currentPassageText.length) {
    ...
    updateLiveHUD();
    scrollPassageDisplay();
    updateCurrentWordHighlight(); // Update word highlight after any character change
}
...
function startTimer() {
    ...
    timerInterval = setInterval(() => {
        ...
        if (correctCharCount > 0) {
            ...
            if (minutes > 0) {
                wpmDisplay.textContent = Math.round((correctCharCount / CHARS_PER_WORD) / minutes);
            } else {
                if (elapsedTimeSeconds > 0) {
                    wpmDisplay.textContent = Math.round((correctCharCount / CHARS_PER_WORD) / (elapsedTimeSeconds / 60));
                } else { wpmDisplay.textContent = '0'; }
            }
        } else if (elapsedTimeSeconds > 0) { wpmDisplay.textContent = '0'; }
    }, 1000);
}

function updateLiveHUD() {
    if (typedCharCount > 0) {
        accuracyDisplay.textContent = `${Math.max(0, Math.round((correctCharCount / typedCharCount) * 100))}`;
    } else { accuracyDisplay.textContent = '0'; }
}
...
function endGame() {
    ...
    let finalAccuracy = 0;
    if (typedCharCount > 0) { finalAccuracy = Math.round((correctCharCount / typedCharCount) * 100); }
